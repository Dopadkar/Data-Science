...
- Модели
- Доработаем данные
- Уберём из обучающей выборки все признаки, которых нет в тестовой
- Разделение данных
- Обучение моделей для rougher
- Обучение моделей для final
- Итоговый ансамбль двух моделей

## Выводы

### Предобработка
Мы провели предобработку данных и заполнили пропуски. Для этого мы установили взаимосвязь между данными с пропусками и без. В тех случаюих, когда пропусков много и корреляция выше 0.75, мы заполнили пропуски на основе корркляций. В остальных случаях пропуски были заполнены медианными значениями.

###  Анализ данных
Анализ данных показал, что:

- в процессе очистки концентрация золота увеличилась, а всех остальных металлов осталасть прежней;
- размеры гранул немного различаются, но это некритично;
- концентрация после отчистки стала более сфокусированной.

### Обучение

Мы обучили по три вида моделей:

- линейная регрессия;
- решающее дерево;
- случайный лес.

Был произведён подбор более двух гиперпараметров. Была проведена кросс валидация. Наилучншие показатели показали модели случайного леса.

Наш ансамбль из двух моделей (двух случайных лесов) дали итоговую sMAPE 9.25778566747889.

Это немного лучше, чем предсказывать по среднему, поскольку итоговая sMAPE по DummyRegressor 9.825704144247414.
